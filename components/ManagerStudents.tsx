...
const ManagerDashboard = ({ selectedMosque }: { selectedMosque: string }) => {
  ...
  const deleteTeacher = (id: number) => {
    if(window.confirm("حذف هذه الحلقة سيؤدي لإزالة بيانات دخول المعلم المرتبط بها. هل أنت متأكد؟")) {
      const all = JSON.parse(localStorage.getItem('afaq_teachers') || '[]');
  ...
  const deleteStudent = (id: number) => {
    if(window.confirm("هل تريد حذف هذا الطالب نهائياً؟")) {
      const all = JSON.parse(localStorage.getItem('afaq_students') || '[]');
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 p-6" dir="rtl">
      ...
      {activeTab === 'students' ? (
        <div className="space-y-6">
          <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] shadow-sm border">
            ...
            <button onClick={() => { setEditingId(null); setNewStudent(studentInitial); setShowStudentModal(true); }} className="bg-emerald-900 text-white px-6 py-2 rounded-xl font-bold">إضافة طالب +</button>
          </div>
            ...
            {students.map(s => (
              <div key={s.id} className="bg-white p-6 rounded-[2rem] border border-emerald-50 shadow-sm flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setNewStudent({...s, tripleName: s.fullName}); setEditingId(s.id); setShowStudentModal(true); }} className="p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 font-bold text-xs">تعديل</button>
                  <button onClick={() => deleteStudent(s.id)} className="p-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 font-bold text-xs">حذف</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] shadow-sm border">
            <h2 className="text-xl font-black text-emerald-900">سجل الحلقات والمعلمين</h2>
            <button onClick={() => { setEditingId(null); setNewTeacher({name:'', username:'', password:'', phone:'', type:'معلم حلقة'}); setShowTeacherModal(true); }} className="bg-emerald-700 text-white px-6 py-2 rounded-xl font-bold">إضافة معلم +</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {teachers.map(t => (
              <div key={t.id} className="bg-white p-6 rounded-[2rem] border border-emerald-50 shadow-sm flex justify-between items-center">
                <div>
                  <h4 className="font-black text-emerald-900">{t.name}</h4>
                  <p className="text-xs text-gray-400 font-bold">المستخدم: {t.username}</p>
                </div>
                <div className="flex gap-2">
                  {/* أزرار الحذف والتعديل المضافة للحلقات */}
                  <button onClick={() => { setNewTeacher(t); setEditingId(t.id); setShowTeacherModal(true); }} className="p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 font-bold text-xs">تعديل</button>
                  <button onClick={() => deleteTeacher(t.id)} className="p-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 font-bold text-xs">حذف</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* مودال الطالب (نفس التصميم السابق الملون) */}
      {showStudentModal && (
        <div className="fixed inset-0 z-[3000] bg-black/60 flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-5xl bg-white rounded-[3rem] p-8 space-y-8 my-8 shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-blue-50/30 p-4 rounded-2xl border border-blue-100 space-y-2 text-center">
                    ...
                    <div className="flex gap-1">
                      <input className="w-1/2 p-1 rounded bg-white border text-[10px]" placeholder="آية" value={newStudent.tathbitVerse} onChange={e => setNewStudent({...newStudent, tathbitVerse: e.target.value})} />
                      <input className="w-1/2 p-1 rounded bg-white border text-[10px]" placeholder="سطر" value={newStudent.tathbitLines} onChange={e => setNewStudent({...newStudent, tathbitLines: e.target.value})} />
                    </div>
                  </div>
                  <div className="bg-amber-50/30 p-4 rounded-2xl border border-amber-100 space-y-2 text-center">
                    ...
                    <div className="flex gap-1">
                      <input className="w-1/2 p-1 rounded bg-white border text-[10px]" placeholder="آية" value={newStudent.murajaahVerse} onChange={e => setNewStudent({...newStudent, murajaahVerse: e.target.value})} />
                      <input className="w-1/2 p-1 rounded bg-white border text-[10px]" placeholder="سطر" value={newStudent.murajaahLinesTarget} onChange={e => setNewStudent({...newStudent, murajaahLinesTarget: e.target.value})} />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-4">
              <button onClick={saveStudent} className="flex-1 py-5 bg-emerald-900 text-white rounded-[2rem] font-black text-xl shadow-xl hover:bg-emerald-800 transition-all">اعتماد وحفظ الطالب</button>
